...
export function useSimulation({ tickMs = 2000, seed = 1, gameKey = 0, initialSave = null } = {}) {
  ...
  const api = useMemo(() => {
    return {
      ...
      applyMeetForeignLeader(countryId) {
        if (!engineRef.current) return
        engineRef.current.applyMeetForeignLeader(countryId)
        const next = engineRef.current.getState()
        setState(next)
        if (next?.regime?.status === 'in_power') try { localStorage.setItem(SAVE_KEY, JSON.stringify(next)) } catch (_) {}
      },
      applyVisitRegion(regionId) {
        if (!engineRef.current) return
        engineRef.current.applyVisitRegion(regionId)
        const next = engineRef.current.getState()
        setState(next)
        if (next?.regime?.status === 'in_power') try { localStorage.setItem(SAVE_KEY, JSON.stringify(next)) } catch (_) {}
      },
      applySecurityBriefingOutcome() {
        if (!engineRef.current) return
        engineRef.current.applySecurityBriefingOutcome()
        const next = engineRef.current.getState()
        setState(next)
        if (next?.regime?.status === 'in_power') try { localStorage.setItem(SAVE_KEY, JSON.stringify(next)) } catch (_) {}
      },
      applyPressConferenceOutcome() {
        if (!engineRef.current) return
        engineRef.current.applyPressConferenceOutcome()
        const next = engineRef.current.getState()
        setState(next)
        if (next?.regime?.status === 'in_power') try { localStorage.setItem(SAVE_KEY, JSON.stringify(next)) } catch (_) {}
      },
