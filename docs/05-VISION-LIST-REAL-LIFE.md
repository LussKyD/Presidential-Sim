...
| **Dossier with country seal** — Dossiers now cover: state visit return (deputy handover), bilateral meeting (country seal), state of the nation, cabinet, visit region, security briefing, press conference, launch infrastructure, budget vote (pass/amend/reject), crisis response (protest/scandal), opening of Parliament, year begins. Event feed "View dossier" and desk "Recent dossiers" open the modal. Economy history recorded once per month (day 1). Event feed colors for all dossier-linked types. | Formal briefs; every major activity leaves a written record (gap 2.1, 2.3). |
